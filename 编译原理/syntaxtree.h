...
public:
    ...
    Kind kind;             // 记录语法树结点的具体类型
    int idnum;             // 记录一个结点中标识符的个数
    std::string name[20];  // 结点中标识符的名字
    SymbTable*
        table[20];  // 指针数组，数组成员是结点中各个标识符在符号表中的入口
    string
        type_name;  // 记录类型名，当结点是声明类型，且类型是由类型标识符表示时有效
    struct {
        struct {
        ...
        struct {
            ...
            VarKind varkind;
            ExpType type;  // 记录语法树结点的检查类型，取值Void, Integer,
                           // Boolean, 为类型检查ExpType类型
        } ExpAttr;  // 记录表达式的属性
    } attr;
