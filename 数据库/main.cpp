...
void testBTree() {
    ...
    for (int i = low; i < up; ++i) {
        // RAND_MAX == INT_MAX
        num.insert(num.end(),
                    // key -> rand() * rand() % 1e8, e -> (float)rand() / rand();
                   Entry(rand() * rand() % 100000000, (float)rand() / rand()));
    }
    // r 中保存树上的元素
    vector<Entry> r;
    cout << "insert values the following:\n" << endl;
    while (!num.empty()) {
        int s = rand() % (num.size());
        auto it = num.begin();
        // move it s step if s is less than 0 then back s step
        advance(it, s);
