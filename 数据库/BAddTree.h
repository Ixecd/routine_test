...
class BAddTree {
public:
    explicit BAddTree(int order = 2);  //当小于2时默认为2阶,即退化为二叉搜索树
    ~BAddTree();
    ...
    // template <class Type>
    // --> func(Type const&) / func(const Type&)
    // --> int const& / const int&
    virtual bool insert(const E&);                     //插入
    virtual bool remove(const E&);                     //删除
...
protected:
    int m_size;  //关键码总数
    int m_order;  // B-树的阶次,至少为2(即退化为二叉搜索树) 创建时指定
                  // 一般不做修改
    BAddTreeNode<K, E>* m_root;           //树根
    BAddTreeNode<K, E>* m_hitParentNode;  //查找访问时命中节点的父亲或者穿透底层后最后一个访问的节点，当树为空时
                                          //其访问后为空
...
inline BAddTreeLeafNode<K, E>* BAddTree<K, E>::search(E const& e) {
    ...
    K k = e.key();
    BAddTreeLeafNode<K, E>* v =
        static_cast<BAddTreeLeafNode<K, E>*>(this->m_root);  // c++式静态转换
    while (!v->isLeaf()) {
        int _s = v->key.size();
        int i = 0;
        for (; i < _s; ++i) {
...
inline bool BAddTree<K, E>::insert(E const& e) {
    ...
    if (v) {  //不允许插入重复值
              //因为插入要上溢下溢可能导致一个节点的2边都存在重复值，导致查找太过复杂
        return false;
    } else {
        K k = e.key();
...
template <typename K, typename E>
inline bool BAddTree<K, E>::remove(const E& e) {
    BAddTreeLeafNode<K, E>* v = this->search(e);
    if (!v) {
        return false;
    }
    int rank = 0;
    while (v->key[rank] != e.key()) ++rank;
    //删除
...
inline void BAddTree<K, E>::solveOverFlow(BAddTreeLeafNode<K, E>* v) {
    while (v->child.size() > m_order)  //上溢
    {
        ...
        if (v->isLeaf()) {  //情况一 叶节点复制上溢点部分并更新链表
            ...
            //关键字处理
            {
                _r->key.insert(_r->key.begin(), v->key.begin() + mid,
                               v->key.end());
                v->key.erase(v->key.begin() + mid + 1, v->key.end());
            }
            //分支处理
            {
                _r->child.insert(_r->child.begin(), v->child.begin() + mid,
                                 v->child.end() - 1);
            ...
            _r = static_cast<BAddTreeLeafNode<K, E>*>(
                new BAddTreeNode<K, E>());  //注：默认有一个空分支
            //关键字处理
            {
                _r->key.insert(_r->key.begin(), v->key.begin() + mid + 1,
                               v->key.end());
                v->key.erase(v->key.begin() + mid + 1, v->key.end());
            }
            //分支处理
            {
                _r->child.insert(_r->child.begin(), v->child.begin() + mid + 1,
                                 v->child.end() - 1);
                v->child.erase(v->child.begin() + mid + 1, v->child.end() - 1);
                _r->child[_r->child.size() - 1] = v->child[v->child.size() - 1];
                v->child.erase(v->child.end() - 1);
        ...
        {
            {
                ...
                if (_r->child[0])  //节点状态统一
                    for (int i = 0; i < _s; ++i)
                        _r->child[i]->parent =
                            static_cast<BAddTreeNode<K, E>*>(_r);
            }
            BAddTreeNode<K, E>* p = v->parent;
            if (!p) {
                this->m_root = p = new BAddTreeNode<K, E>();
                p->child[0] = v;
                v->parent = p;  //上溢点为根，建立新根
            }
...
inline void BAddTree<K, E>::solveUnderFlow(BAddTreeLeafNode<K, E>* v) {
    ...
    while (v->child.size() < sep)    //下溢
    {
        ...
        if (!p) {  //根节点最小关键码为1
            if (v->key.empty() &&
                ...
                if (v->isLeaf()) {  //叶子节点则更新链表
                    ...
                    BAddTreeNode<K, E>* _n =
                        static_cast<BAddTreeNode<K, E>*>(v);
        ...
        //分2块 叶子节点及内部节点 内部节点同B树
        //叶子节点则删掉B树中的下拉值或借一个同时替换掉父节点(注：替换)
        if (v->isLeaf()) {
            //优先向左合并 满足完全二叉树性
            //情况1：左可借
            if (rank > 0) {
                BAddTreeLeafNode<K, E>* _left =
                    static_cast<BAddTreeLeafNode<K, E>*>(
                        p->child[rank - 1]);  //左兄弟
                if (_left->child.size() > sep) {
                    v->child.insert(
                        v->child.begin(),
                        _left->child[_left->child.size() - 1]);  //借一个分支
            ...
            if (rank < p->child.size() - 1) {
                BAddTreeLeafNode<K, E>* _right =
                    static_cast<BAddTreeLeafNode<K, E>*>(
                        p->child[rank + 1]);  //右兄弟
                if (_right->child.size() > sep) {
                    v->child.insert(v->child.end(),
                                    _right->child[0]);  //借一个分支
            ...
            if (rank > 0) {  //优先向左合并 满足完全二叉树性
                ...
                v->child.erase(
                    v->child
                        .begin());  //首先删除一个多余分支，这里根据右归属分支原则删除，当然也可以随便删除
                _left->child.insert(_left->child.end(), v->child.begin(),
                                    v->child.end());
                ;  //合并分支
                _left->key.insert(_left->key.end(), v->key.begin(),
                                  v->key.end());  //合并关键码
                //删除父关键码(不下拉)
                ...
                _right->child.erase(
                    _right->child
                        .begin());  //首先删除一个多余分支，这里根据右归属分支原则删除，当然也可以随便删除
                _right->child.insert(_right->child.begin(), v->child.begin(),
                                     v->child.end());
                ;  //合并分支
                _right->key.insert(_right->key.begin(), v->key.begin(),
                                   v->key.end());  //合并关键码
                //删除父关键码(不下拉)
                p->child.erase(p->child.begin() + rank);
            ...
            if (rank < p->child.size() - 1) {
                ...
                if (_right->child.size() > sep) {
                    ...
                    if (v->child[v->child.size() - 1])
                        v->child[v->child.size() - 1]->parent = v;  //向上联接
                    _right->child.erase(_right->child.begin());     //删除
            ...
            if (rank > 0) {  //优先向左合并 满足完全二叉树性
                ...
                {
                    BAddTreeNode<K, E>* _t =
                        static_cast<BAddTreeNode<K, E>*>(v);  //析构内部节点
